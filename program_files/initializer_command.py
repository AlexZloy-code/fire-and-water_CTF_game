...
map = Map(FILE_NAME)  # передает классу Map информацию об уровне
draw = Draw()  # класс отрисовки
menu = Menu()  # класс меню
music = Music()  # класс музыки
map.player_initializer(CELLS['9'], Player, players)  # инициализирует всех персонажей
map.button_fence_initializer(BUTTON_INITIALIZER, Button, buttons, Fence, fences)  # инициализирует кнопки и баррикады
map.gate_initializer(Gate, gates)  # инизицлизирует враща
map.decoration_initializer(Decoration, decorations)  # инициализирует декорацию
